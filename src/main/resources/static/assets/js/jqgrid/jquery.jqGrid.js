...
$.extend($.jgrid,{
	...
	styleUI : {
		jQueryUI : {
			...
			navigator : {
				icon_edit_nav : "icon-pencil blue",             //编辑小图标
				icon_add_nav : "icon-plus-sign purple",         //新增小图标
				icon_del_nav : "icon-trash red",                //删除小图标
				icon_search_nav : "icon-search orange",         //搜索小图标
				icon_refresh_nav : "icon-refresh green",        //刷新小图标
				icon_view_nav : "icon-zoom-in grey",            //查看小图标
		...
		Bootstrap : {
			...
			base : {
				...
				pgButtonBox : "",
				icon_first : "icon-double-angle-left bigger-140", //首页
				icon_prev : "icon-angle-left bigger-140",         //上一页
				icon_next: "icon-angle-right bigger-140",         //下一页
				icon_end: "icon-double-angle-right bigger-140",   //尾页
			...
			grouping : {
				icon_plus : 'icon-chevron-right',    //多级表格右按钮
				icon_minus : 'icon-chevron-down'     //多级表格下按钮
			},
			filter : {
			...
			fmatter : {
				icon_edit : "icon-pencil green",
				icon_add : "icon-plus-sign purple",
				icon_save : "icon-ok-sign blue",
				icon_cancel : "icon-remove-sign orange",
				icon_del : "icon-trash red"
			},
			colmenu : {
				menu_widget : '',
				input_checkbox : "",
...
//Helper functions
	...
	viewModal : function (selector,o){
		o = $.extend({
			...
			overlay: 10,
			modal: false,
			overlayClass : 'ui-widget-overlay', // 固定的
	...
	info_dialog : function(caption, content,c_b, modalopt) {
		...
		// 如果不提供id，我就把它 info_button_+ 索引数组中 - i.e info_button_0,info_button_1...
		};
						...
						success: function(data){
							...
							if(a) {
								...
								$("option",elem).each(function(i){
									...
									// 修复IE8 / IE7选择第一项的问题 multiple=true
...
// 日期验证的Javascript
	checkDate : function (format, date) {
		...
		// 2月有29天任何一年都不能被四整除,除一世纪的年不也能被400整除
		...
		*/
		...
		this.createTableForGroup = function(group, parentgroup) {
			...
			inputAddRule.on('click',function() {
				...
				// 只显示serchable和serchhidden = true字段
				...
				that.reDraw(); // html已经改变,力重绘
				// 目前没有改变了规则,所以这不会触发onchange事件
				return false;
			});
		...
		this.createTableRowForRule = function(rule, group ) {
			// 保存当前实体在一个变量中,以便它可以引用anonimous方法调用
			...
			// dropdown for: choosing field 下拉选择：选择字段
			...
			cm = p.columns[j];
			// 在这里创建,因此它可以referentiated onchange事件
			...
			// dropdown for: choosing operator 下拉选择：选择运营商
			...
			ruleOperatorSelect.on('change',function() {
				...
				if (rule.op === "nu" || rule.op === "nn") { // 禁用操作符“为空”和“不空”
					rule.data = "";
			...
			function fillData(rowid,obj,fmid){
				...
				$('td[role="gridcell"]',tre).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn' && cm[i].editable===true) {
						...
						nm = $.jgrid.jqID(nm);
						switch (cm[i].edittype) {   //edittype属性
							case "password":
							case "text":
							case "button" :
							case "image":
		...
		return this.each(function() {
			...
			for(i = 0; i<clone; i++) {
				...
				if (o.add) {
					...
					//$(tbd).append("<div class='ui-pg-div'><a class='btn btn-purple btn-xs' href='javascript:;'><span class='"+commonstyle.icon_base +" " +o.addicon+"'></span>"+o.addtext+"<span>新增</span></a></div>");
				...
				if(o.add || o.edit || o.del || o.view ) {$("tr",navtbl).append(sep);}//小图标中间的分隔线
				pSearch = pSearch || {};//搜索
				if (o.search) {
					tbd = $("<td class='ui-pg-button ui-corner-all"+commonstyle.cornerall+"'></td>");
			...
			if($t.p.storeNavOptions) {
				$t.p.navOptions = o;
				$t.p.editOptions = pEdit;//编辑
				$t.p.addOptions = pAdd;//新增
				$t.p.delOptions = pDel;//删除
				$t.p.searchOptions = pSearch;//搜索
				$t.p.viewOptions = pView;//查看
				$t.p.navButtons =[];
			}
